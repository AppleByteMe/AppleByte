                              ...
                              std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s ABY additional fees, %d delta bytes\n",
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (block.nVersion >= 3 && IsSuperMajority(3, pindex->pprev, chainparams.GetConsensus().nMajorityEnforceBlockUpgrade, chainparams.GetConsensus())) {
        if (chain == CBaseChainParams::TESTNET)
            LogPrintf("Consensus fork BIP66 enabled\n");
    ...
    if (block.nVersion >= 4 && IsSuperMajority(4, pindex->pprev, chainparams.GetConsensus().nMajorityEnforceBlockUpgrade, chainparams.GetConsensus())) {
        if (chain == CBaseChainParams::TESTNET)
            LogPrintf("Consensus fork BIP65 enabled\n");
    ...
    if (VersionBitsState(pindex->pprev, chainparams.GetConsensus(), Consensus::DEPLOYMENT_CSV, versionbitscache) == THRESHOLD_ACTIVE) {
        if (chain == CBaseChainParams::TESTNET)
            LogPrintf("Consensus fork CSV BIP68/BIP112 enabled\n");
    ...
    if (IsWitnessEnabled(pindex->pprev, chainparams.GetConsensus())) {
        if (chain == CBaseChainParams::TESTNET)
            LogPrintf("Consensus fork SEGWIT enabled\n");
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, CBlockIndex * const pindexPrev, int64_t nAdjustedTime)
{
    ...
    if (block.GetBlockTime() <= pindexPrev->GetBlockTime() - 15 * 60)
        return state.Invalid(false, REJECT_INVALID, "time-too-old", "block timestamp too early to compare to previous block");
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    if (VersionBitsState(pindexPrev, consensusParams, Consensus::DEPLOYMENT_CSV, versionbitscache) == THRESHOLD_ACTIVE) {
		std::string chain = ChainNameFromCommandLine();
		if (chain == CBaseChainParams::TESTNET)
            LogPrintf("Consensus fork CSV BIP113 enabled\n");
